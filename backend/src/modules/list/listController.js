...
import gameModel from "../promo/promoModel.js";
...
const add = async (req, res) => {
    ...
    if (!VALID_TYPES.includes(tipo_lista)) {
        return res.status(400).json({ message: `tipo_lista deve ser um dos: ${VALID_TYPES.join(", ")}.` });
    }
    ...
    try {
        ...
        if (!jogo) return res.status(404).json({ message: "Jogo não encontrado no banco." });

        const existing = await listModel.findByUserAndGame(usuario_id, jogo_id);
        if (existing) return res.status(409).json({ message: "Jogo já está na sua lista." });

        await listModel.create({ usuario_id, jogo_id, tipo_lista, prioridade });
        return res.status(201).json({ message: "Jogo adicionado à lista com sucesso." });
    } catch (error) {
...
const listByType = async (req, res) => {
    ...
    if (!VALID_TYPES.includes(tipo)) {
        return res.status(400).json({ message: `tipo deve ser um dos: ${VALID_TYPES.join(", ")}.` });
    }
...
const moveToList = async (req, res) => {
    ...
    try {
        const existing = await listModel.findByUserAndGame(usuario_id, jogo_id);
        if (!existing) return res.status(404).json({ message: "Jogo não encontrado na sua lista." });
...
const updatePriority = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: "Prioridade atualizada com sucesso." });
    } catch (error) {
...
const remove = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: "Jogo removido da lista com sucesso." });
    } catch (error) {
