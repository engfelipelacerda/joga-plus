...
const create = async (req, res) => {
    ...
    if (comentario && comentario.length > 500) {
        return res.status(400).json({ message: "Comentário não pode exceder 500 caracteres." });
    }
    ...
    try {
        ...
        if (!inList || inList.tipo_lista !== "jogados") {
            return res.status(403).json({ message: "Você só pode avaliar jogos da lista 'jogados'." });
        }

        // Verifica se já existe avaliação
        const existing = await assessmentModel.findByUserAndGame(usuario_id, jogo_id);
        if (existing) {
            return res.status(409).json({ message: "Você já avaliou esse jogo. Use o método de edição." });
        }
...
const getByGame = async (req, res) => {
    ...
    try {
        const assessment = await assessmentModel.findByUserAndGame(usuario_id, jogo_id);
...
const update = async (req, res) => {
    ...
    try {
        const existing = await assessmentModel.findByUserAndGame(usuario_id, jogo_id);
        if (!existing) return res.status(404).json({ message: "Avaliação não encontrada." });
